...
export const POST = async (request: Request) => {
  ...
  try {
    ...
    if(!username || !name || !lastname || !email || !company || !role || !password) {
     ...
     return NextResponse.json({ message: `Пользователь с именем ${name} ${lastname} создан!`, status: 'success' }, { status: 200 })

  } catch (error: Error | any) {

    return NextResponse.json({ message: `Ошибка при создании пользователя ${error.message}`}, { status: 500 })
